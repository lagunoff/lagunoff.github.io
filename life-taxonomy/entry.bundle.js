...
var _getDynamicStyles = __webpack_require__(49);
...
var _SheetsManager = __webpack_require__(50);
...
var _Jss = __webpack_require__(57);
...
var _toCss = __webpack_require__(17);
...
var _linkRule = __webpack_require__(31);
...
var _escape = __webpack_require__(55);
...
var OptionBase = /** @class */ (function () {
    function OptionBase() {
    }
    /** Apply function `f` to the underlying value */
    OptionBase.prototype.map = function (f) {
        var $this = this;
        switch ($this.tag) {
    ...
    OptionBase.prototype.chain = function (f) {
        var $this = this;
        switch ($this.tag) {
            case 'None': return $this;
    ...
    OptionBase.prototype.fold = function (fromNone, fromSome) {
        var $this = this;
        switch ($this.tag) {
    ...
    OptionBase.prototype.withDefault = function (fromNone) {
        return this.fold(fromNone, function (x) { return x; });
    };
    return OptionBase;
}());
...
function some(a) { return new Some(a); }
exports.some = some;
exports.of = some;
...
Object.defineProperty(exports, "__esModule", { value: true });
var jss_1 = __webpack_require__(3);
var jss_preset_default_1 = __webpack_require__(69);
...
var theme = function () {
    ...
    var base = colors[0][0];
    var palette = { first: colors[0][0], second: colors[1][1], colors: colors };
    var spacing = { unit: 8 };
    var toolbar = { height: 30 + spacing.unit * 2 };
    return { spacing: spacing, toolbar: toolbar, palette: palette };
}();
...
var Application = /** @class */ (function (_super) {
    ...
    function Application(props) {
        ...
        _this.handlePopState = function () {
            var breadcrumbs = _this.parseLocation(window.location);
            _this.setState({ breadcrumbs: breadcrumbs, pending: true });
            _this.fetchContent(breadcrumbs[0])
                .then(function (text) { return _this.setState({ text: text, pending: false }); })
                .catch(function () { return _this.setState({ pending: false }); });
        };
        var breadcrumbs = _this.parseLocation(window.location);
        _this.state = { breadcrumbs: breadcrumbs, text: '', pending: true };
        _this.fetchContent(breadcrumbs[0])
            .then(function (text) { return _this.setState({ text: text, pending: false }); })
            .catch(function () { return _this.setState({ pending: false }); });
        return _this;
    ...
    Application.prototype.componentDidMount = function () {
        var prevValue = window.onpopstate;
        window.onpopstate = this.handlePopState;
        this.unlisten = function () { return (window.onpopstate = prevValue); };
    };
    Application.prototype.componentWillUnmount = function () {
        this.unlisten && this.unlisten();
    };
    Application.prototype.parseLocation = function (location) {
        var path = !location.hash ? '' : location.hash[0] === '#' ? location.hash.slice(1) : location.hash;
        var breadcrumbs = exports.parser.parseAll(path);
        var routeNotFound = exports.parser.parse(exports.parser.print({ tag: 'Home' }));
    ...
    Application.prototype.renderBreadcrumbs = function () {
        ...
        return intersperse(breadcrumbs.map(function (r, idx) { return React.createElement("a", { key: idx, href: '#' + exports.parser.print(r) }, r.title); }).reverse(), React.createElement("span", null, " \u2192 "));
        function intersperse(xs, x) {
            return xs.reduce(function (acc, a, idx) { return (idx && acc.push(React.cloneElement(x, { key: 'glue-' + idx })), acc.push(a), acc); }, []);
        }
    };
    Application.prototype.fetchContent = function (route) {
        if ('wikiLink' in route)
            return fetchWikiContent(route.wikiLink);
        else
            return Promise.resolve(readme);
    };
    Application.prototype.render = function () {
        var _a = this.state, __html = _a.text, pending = _a.pending, breadcrumbs = _a.breadcrumbs;
        return React.createElement("div", { className: classes.root },
            React.createElement("div", { className: classes.columns },
                React.createElement(LifeTreeMenu_1.default, { className: classes.menu, route: this.state.breadcrumbs[0] }),
                React.createElement("div", { className: classes.right },
                    React.createElement(Breadcrumbs_1.default, { embeddedClassName: classes.breadcrumbs, breadcrumbs: breadcrumbs }),
                    React.createElement("div", { className: classes.wiki, dangerouslySetInnerHTML: { __html: __html } }),
                    pending && React.createElement(Spinner_1.default, { className: classes.spinner, overlay: true }))));
    };
    return Application;
}(React.Component));
function fetchWikiContent(wikiLink) {
    return new Promise(function (resolve, reject) {
        var xhr = new XMLHttpRequest();
        xhr.addEventListener('error', function (e) { return reject(e); });
        xhr.addEventListener('timeout', function (e) { return reject(e); });
        xhr.addEventListener('load', function () {
            var jsonString = xhr.response || xhr.responseText;
            try {
                var json = JSON.parse(jsonString);
                var wikiText = processWikiContent(json.parse.text['*']);
                resolve(wikiText);
            }
            catch (e) {
                reject(e);
...
function processWikiContent(htmlString) {
    var html = createElementFromHTML(htmlString);
    if (!html)
        return htmlString;
...
exports.styles = function () {
    var unit = theme_1.default.spacing.unit, toolbar = theme_1.default.toolbar;
    return {
        root: {
            height: '100%',
            '& pre': {
        ...
        wiki: __assign({}, theme_1.linkStyle, { fontSize: 14, '& .infobox': { float: 'right', margin: [unit * 2, 0, unit * 2, unit * 2] } }),
        right: {
            height: '100%',
            position: 'relative',
            overflowY: 'scroll',
...
var classes = jss_1.default.createStyleSheet(exports.styles).attach().classes;
var init = function () {
    var container = document.createElement('div');
    document.body.appendChild(container);
    ReactDOM.render(React.createElement(Application, null), container);
};
...
Object.defineProperty(exports, "__esModule", { value: true });
var option_1 = __webpack_require__(9);
...
var ParserState = /** @class */ (function () {
    function ParserState(segments, params, idx) {
...
(function (ParseOptions) {
    ParseOptions[ParseOptions["OnlyFirstMatch"] = 1] = "OnlyFirstMatch";
    ParseOptions[ParseOptions["AllSegmentsConsumed"] = 2] = "AllSegmentsConsumed";
})(ParseOptions = exports.ParseOptions || (exports.ParseOptions = {}));
var OnlyFirstMatch = ParseOptions.OnlyFirstMatch, AllSegmentsConsumed = ParseOptions.AllSegmentsConsumed;
/**
...
var Parser = /** @class */ (function () {
    ...
    Parser.prototype.parse = function (url) {
        var results = doParse(this.rules, prepareState(url), OnlyFirstMatch);
        return results.length ? results[0][0] : null;
    };
    /** Convert result of parsing back into url. Reverse of `parse` */
    Parser.prototype.print = function (route) {
        return assembleChunks(doPrint(this.rules, route));
    };
    /** Return all matches */
    Parser.prototype.parseAll = function (url) {
        var results = doParse(this.rules, prepareState(url), 0x0).sort(compareFn);
        var output = [];
        var idx = -1;
        for (var _i = 0, results_1 = results; _i < results_1.length; _i++) {
            var _a = results_1[_i], route = _a[0], state = _a[1];
            if (idx === state.idx)
                continue;
            output.push(route);
            idx = state.idx;
     ...
     * matter
     */
    Parser.prototype.path = function (path) {
        var segments = path.split('/').filter(function (x) { return x !== ''; });
        this.rules.push({ tag: 'Path', segments: segments });
        return this;
    };
    /**
     ...
     */
    Parser.prototype.segment = function (key, adapter) {
        this.rules.push({ tag: 'Segment', key: key, adapter: adapter });
        return this;
    };
    /**
     ...
     * are adapters
     */
    Parser.prototype.params = function (params) {
        this.rules.push({ tag: 'Params', params: params });
        return this;
    };
    /**
     ...
     */
    Parser.prototype.concat = function (that) {
        var _this = this;
        that.rules.forEach(function (x) { return _this.rules.push(x); });
        return this;
    };
    /**
     ...
     * @param that Another `Parser`
     */
    Parser.prototype.embed = function (key, that) {
        this.rules.push({ tag: 'Embed', key: key, rules: that.rules.slice() });
        return this;
    };
    /**
     ...
     */
    Parser.prototype.extra = function (payload) {
        this.rules.push({ tag: 'Extra', payload: payload });
        return this;
    };
...
function tag(tag) {
    return new Parser([{ tag: 'Extra', payload: { tag: tag } }]);
}
exports.tag = tag;
/** @see `Parser.prototype.path` */
function path(segmentsStr) {
    var segments = segmentsStr.split('/').filter(function (x) { return x !== ''; });
    return new Parser([{ tag: 'Path', segments: segments }]);
}
exports.path = path;
/** @see `Parser.prototype.segment` */
function segment(key, adapter) {
    return new Parser([{ tag: 'Segment', key: key, adapter: adapter }]);
}
exports.segment = segment;
/** @see `Parser.prototype.extra` */
function extra(payload) {
    return new Parser([{ tag: 'Extra', payload: payload }]);
}
exports.extra = extra;
/** @see `Parser.prototype.params` */
function params(params) {
    return new Parser([{ tag: 'Params', params: params }]);
}
exports.params = params;
/**
 * Implement custom parser
 */
function custom(parse, print) {
    return new Parser([{ tag: 'Custom', parse: parse, print: print }]);
}
exports.custom = custom;
function oneOf() {
    var parsers = Array.isArray(arguments[0]) ? arguments[0] : arguments;
    var tags = {};
    for (var i = 0; i < parsers.length; i++) {
        var tag_1 = lookupTag(parsers[i]);
        if (tag_1)
            tags[tag_1] = parsers[i].rules.slice().sort(compareFn);
        else
            throw new Error("oneOf: argument #" + (i + 1) + " wasn't provided with a tag");
    }
    var prefixTrie = buildTrie(tags);
    return new Parser([{ tag: 'OneOf', tags: tags, prefixTrie: prefixTrie }]);
    function lookupTag(parser) {
        for (var _i = 0, _a = parser.rules; _i < _a.length; _i++) {
            var rule = _a[_i];
            if (rule.tag === 'Extra' && typeof (rule.payload['tag']) === 'string')
                return rule.payload['tag'];
        }
        return null;
    }
    function compareFn(a, b) {
        var aWeight = a.tag === 'Path' || a.tag === 'Segment' ? -1 : 0;
        var bWeight = b.tag === 'Path' || b.tag === 'Segment' ? -1 : 0;
        return aWeight - bWeight;
    }
}
exports.oneOf = oneOf;
// construct state from relative url
function prepareState(url) {
    var _a = url.split('?'), path = _a[0], query = _a[1];
    var segments = path.split('/').filter(function (x) { return x !== ''; }).map(decodeURIComponent);
    var params = (query || '').split('&').filter(function (x) { return x !== ''; }).reduce(function (acc, pair) {
        var _a = pair.split('=').map(decodeURIComponent), key = _a[0], value = _a[1];
        acc[key] = value || '';
        return acc;
...
function assembleChunks(chunks) {
    var segments = chunks[0], params = chunks[1];
    var query = Object.keys(params).map(function (key) {
        var _a = [prettyUriEncode(key), prettyUriEncode(params[key])], k = _a[0], v = _a[1];
        return k + (v ? '=' + v : '');
    }).join('&');
...
// do actual parsing
function doParse(rules, state, options) {
    if (options === void 0) { options = AllSegmentsConsumed; }
    if (rules.length === 0)
        return [];
    var results = [[{}, state.clone()]];
    for (var _i = 0, rules_1 = rules; _i < rules_1.length; _i++) {
        var method = rules_1[_i];
        if (method.tag === 'Params' || method.tag === 'Segment' || method.tag === 'Path' || method.tag === 'Extra') {
            ...
            var i = 0;
            while (i < results.length) {
                var replacements = parseMultiple(method, results[i][0], results[i][1]);
                results.splice.apply(results, [i, 1].concat(replacements));
    ...
    function parseSingle(rule, output, state) {
        var segments = state.segments, params = state.params, idx = state.idx;
        switch (rule.tag) {
            case 'Params': {
                for (var key in rule.params) {
                    if (!rule.params.hasOwnProperty(key))
                        continue;
                    var item = rule.params[key];
                    var adapter = item.tag === 'NamedAdapter' ? item.adapter : item;
                    var paramKey = item.tag === 'NamedAdapter' ? item.name : key;
                    var maybeValue = adapter.applyPartial(params.hasOwnProperty(paramKey) ? option_1.some(params[paramKey]) : option_1.none);
                    if (maybeValue.tag === 'None')
                        return false;
            ...
            case 'Segment': {
                if (idx === segments.length)
                    return false;
                var segment_1 = segments[idx];
                var result = rule.adapter.applyTotal(segment_1);
                if (result.tag === 'None')
                    return false;
            ...
            case 'Path': {
                ...
                for (var i = 0; i < rule.segments.length; i++)
                    if (segments[idx + i] !== rule.segments[i]) {
                        mathes = false;
                        break;
                    }
                if (!mathes)
    ...
    function parseMultiple(rule, prevOutput, prevState) {
        switch (rule.tag) {
            case 'OneOf': {
                var output = [];
                var trie = rule.prefixTrie || { '': Object.keys(rule.tags).map(function (k) { return rule.tags[k]; }) };
                var iter = trie;
                var idx = prevState.idx;
                var parents = [];
                var segments = prevState.segments;
                do {
                    parents.push(iter);
                    var nextSegment = segments[idx++];
                    if (!iter.hasOwnProperty(nextSegment))
                        break;
                    iter = iter[nextSegment];
                } while (1);
                for (var i = parents.length - 1; i >= 0; i--) {
                    for (var _i = 0, _a = parents[i]['']; _i < _a.length; _i++) {
                        var rules_2 = _a[_i];
                        for (var _b = 0, _c = doParse(rules_2, prevState, options); _b < _c.length; _b++) {
                            var pair = _c[_b];
                            var route = pair[0], state_1 = pair[1];
                            Object.assign(route, prevOutput);
                            if (!(options & OnlyFirstMatch))
                                output.push(pair);
                            if ((options & OnlyFirstMatch) && state_1.idx === state_1.segments.length)
                                return [pair];
                        }
                    }
                }
            ...
            case 'Custom': {
                var output = rule.parse(prevState);
                for (var _d = 0, output_1 = output; _d < output_1.length; _d++) {
                    var pair = output_1[_d];
                    var route = pair[0], state_2 = pair[1];
                    Object.assign(route, prevOutput);
                    if ((options & OnlyFirstMatch) && state_2.idx === state_2.segments.length)
                        return [pair];
                }
                return (options & OnlyFirstMatch) ? [] : output;
            }
            case 'Embed': {
                var output = doParse(rule.rules, prevState, options);
                for (var i in output) {
                    output[i][0] = Object.assign((_e = {}, _e[rule.key] = output[i][0], _e), prevOutput);
                }
                return output;
            }
        }
...
// do printing
function doPrint(rules, route) {
    var output = [[], {}];
    for (var _i = 0, rules_3 = rules; _i < rules_3.length; _i++) {
        var rule = rules_3[_i];
        printHelper(rule, route, output);
    }
    return output;
    function printHelper(rule, route, output) {
        var segments = output[0], params = output[1];
        switch (rule.tag) {
            case 'Params': {
                for (var key in rule.params) {
                    if (!rule.params.hasOwnProperty(key))
                    ...
                    var adapter = item.tag === 'NamedAdapter' ? item.adapter : item;
                    var paramKey = item.tag === 'NamedAdapter' ? item.name : key;
                    var maybeValue = adapter.unapplyPartial(route[key]);
                    if (maybeValue.tag === 'Some')
                        params[paramKey] = maybeValue.value;
                }
                return void 0;
            }
            case 'Segment': {
            ...
            case 'Path': {
                rule.segments.forEach(function (x) { return segments.push(x); });
                return void 0;
            }
            case 'OneOf': {
                for (var _i = 0, _a = rule.tags[route['tag']]; _i < _a.length; _i++) {
                    var nestedRules = _a[_i];
            ...
            case 'Custom': {
                for (var _b = 0, _c = rule.print(route); _b < _c.length; _b++) {
                    var chunks = _c[_b];
                    chunks[0].forEach(function (x) { return segments.push(x); });
                    Object.assign(params, chunks[1]);
                }
                return void 0;
            }
            case 'Embed': {
                for (var _d = 0, _e = rule.rules; _d < _e.length; _d++) {
                    var nestedRules = _e[_d];
...
// -- helpers --
function buildTrie(tags) {
    var trie = { '': [] };
    for (var k in tags) {
        var iter = trie;
        for (var j = 0; j < tags[k].length; j++) {
            var rule = tags[k][j];
            if (rule.tag !== 'Path')
                break;
            for (var _i = 0, _a = rule.segments; _i < _a.length; _i++) {
                var s = _a[_i];
                iter[s] = iter[s] || { '': [] };
                iter = iter[s];
            }
...
// custom uri encoding
var prettyUriEncode = function () {
    var keepIntact = [':', ','].reduce(function (acc, c) { return (acc[encodeURIComponent(c)] = c, acc); }, {});
    var re = new RegExp(Object.keys(keepIntact).map(escapeRegExp).join('|'), 'g');
    return function (str) { return encodeURIComponent(str).replace(re, function (sub) { return keepIntact[sub]; }); };
}();
...
Object.defineProperty(exports, "__esModule", { value: true });
var isequal_1 = __webpack_require__(47); // tslint:disable-line:import-name
var option_1 = __webpack_require__(9);
// Base class with instance methods
var AdapterBase = /** @class */ (function () {
     ...
     * ```
     */
    AdapterBase.prototype.withName = function (name) {
        var $this = this;
        if ($this.tag === 'NamedAdapter')
            return new NamedAdapter(name, $this.adapter);
        return new NamedAdapter(name, $this);
    };
    AdapterBase.prototype.withDefault = function (defaultVal) {
        var $this = this;
        switch ($this.tag) {
            case 'TotalAdapter': {
                var applyTotal = function (s) { return option_1.some($this.applyTotal(s).withDefault(defaultVal)); };
                var unapplyTotal = function (a) { return $this.unapplyTotal(a); };
                return new TotalAdapter(applyTotal, unapplyTotal);
            }
            case 'PartialAdapter': {
                var applyPartial = function (s) { return option_1.some($this.applyPartial(s).withDefault(defaultVal)); };
                var unapplyPartial = function (aOrB) { return isequal_1.default(aOrB, defaultVal) ? option_1.none : $this.unapplyPartial(aOrB); };
            ...
            case 'NamedAdapter': {
                return new NamedAdapter($this.name, $this.adapter.withDefault(defaultVal));
            }
        }
    };
    AdapterBase.prototype.dimap = function (f, g) {
        var $this = this;
        switch ($this.tag) {
            case 'TotalAdapter': {
                var applyTotal = function (s) { return $this.applyTotal(s).map(f); };
                var unapplyTotal = function (b) { return $this.unapplyTotal(g(b)); };
                return new TotalAdapter(applyTotal, unapplyTotal);
            }
            case 'PartialAdapter': {
                var applyPartial = function (s) { return $this.applyPartial(s).map(f); };
                var unapplyPartial = function (b) { return $this.unapplyPartial(g(b)); };
                return new PartialAdapter(applyPartial, unapplyPartial);
            }
            case 'PartialAndTotalAdapter': {
                var applyTotal = function (s) { return $this.applyTotal(s).map(f); };
            ...
            case 'NamedAdapter': {
                return new NamedAdapter($this.name, $this.adapter.dimap(f, g));
            }
        }
    };
    return AdapterBase;
 ...
 */
var TotalAdapter = /** @class */ (function (_super) {
    __extends(TotalAdapter, _super);
    function TotalAdapter(applyTotal, unapplyTotal) {
        var _this = _super.call(this) || this;
        _this.applyTotal = applyTotal;
        _this.unapplyTotal = unapplyTotal;
        _this.tag = 'TotalAdapter';
        return _this;
    }
    return TotalAdapter;
}(AdapterBase));
exports.TotalAdapter = TotalAdapter;
/**
 * `PartialAdapter<A>` describes mutual correspondence between
 * `Option<string>` and `A`. These adapters are used in `r.param`
 */
var PartialAdapter = /** @class */ (function (_super) {
    __extends(PartialAdapter, _super);
    function PartialAdapter(applyPartial, unapplyPartial) {
        var _this = _super.call(this) || this;
        _this.applyPartial = applyPartial;
        _this.unapplyPartial = unapplyPartial;
        _this.tag = 'PartialAdapter';
        return _this;
    }
    return PartialAdapter;
}(AdapterBase));
exports.PartialAdapter = PartialAdapter;
/** Combination of `TotalAdapter<A>` and `PartialAdapter<A>` */
var PartialAndTotalAdapter = /** @class */ (function (_super) {
    __extends(PartialAndTotalAdapter, _super);
    function PartialAndTotalAdapter(applyTotal, unapplyTotal, applyPartial, unapplyPartial) {
        var _this = _super.call(this) || this;
        _this.applyTotal = applyTotal;
        _this.unapplyTotal = unapplyTotal;
        _this.applyPartial = applyPartial;
        _this.unapplyPartial = unapplyPartial;
        _this.tag = 'PartialAndTotalAdapter';
        return _this;
    }
    return PartialAndTotalAdapter;
}(AdapterBase));
exports.PartialAndTotalAdapter = PartialAndTotalAdapter;
/** Contains another adapter and its name */
var NamedAdapter = /** @class */ (function (_super) {
    __extends(NamedAdapter, _super);
    function NamedAdapter(name, adapter) {
        var _this = _super.call(this) || this;
        _this.name = name;
        _this.adapter = adapter;
        _this.tag = 'NamedAdapter';
        return _this;
    }
    return NamedAdapter;
}(AdapterBase));
exports.NamedAdapter = NamedAdapter;
/** Strings */
var stringAdapter = new PartialAdapter(function (x) { return x; }, // applyPartial
option_1.some);
exports.string = stringAdapter;
/** Non-empty strings */
var nestringAdapter = new PartialAndTotalAdapter(function (x) { return x !== '' ? option_1.some(x) : option_1.none; }, // applyTotal
function (// applyTotal
x) { return x; }, // unapplyTotal
function (// unapplyTotal
x) { return x.chain(function (str) { return str ? option_1.some(str) : option_1.none; }); }, // applyPartial
option_1.some);
exports.nestring = nestringAdapter;
/** Natural numbers (0, 1, 2, ...) */
var natAdapter = new PartialAndTotalAdapter(function (str) { var i = parseInt(str); return !isNaN(i) && i >= 0 ? option_1.some(i) : option_1.none; }, // applyTotal
String, // unapplyTotal
function (// unapplyTotal
x) { return x.chain(function (str) { var i = parseInt(str); return !isNaN(i) && i >= 0 ? option_1.some(i) : option_1.none; }); }, // applyPartial
function (// applyPartial
x) { return option_1.some(String(x)); });
exports.nat = natAdapter;
/** Integers (..., -1, 0, 1, 2, ...) */
var intAdapter = new PartialAndTotalAdapter(function (str) { var i = parseInt(str); return !isNaN(i) ? option_1.some(i) : option_1.none; }, // applyTotal
String, // unapplyTotal
function (// unapplyTotal
x) { return x.chain(function (str) { var i = parseInt(str); return !isNaN(i) ? option_1.some(i) : option_1.none; }); }, // applyPartial
function (// applyPartial
x) { return option_1.some(String(x)); });
exports.int = intAdapter;
/** Dates as iso-8601 */
var iso8601Adapter = new PartialAndTotalAdapter(function (str) { var d = new Date(str); return isNaN(d.valueOf()) ? option_1.none : option_1.some(d); }, // applyTotal
function (// applyTotal
d) { return d.toISOString(); }, // unapplyTotal
function (// unapplyTotal
x) { return x.chain(function (str) { var d = new Date(str); return isNaN(d.valueOf()) ? option_1.none : option_1.some(d); }); }, // applyPartial
function (// applyPartial
d) { return option_1.some(d.toISOString()); });
exports.date = iso8601Adapter;
/** Booleans */
var booleanAdapter = new PartialAndTotalAdapter(function (x) { return x === '' || x === 'false' || x === '0' || x === 'off' ? option_1.some(false) : option_1.some(true); }, // applyTotal
function (// applyTotal
x) { return x ? 'true' : 'false'; }, // unapplyTotal
function (// unapplyTotal
x) { return x.fold(option_1.some(false), function () { return option_1.some(true); }); }, // applyPartial
function (// applyPartial
x) { return x ? option_1.some('') : option_1.none; });
exports.boolean = booleanAdapter;
/**
...
function array(adapter) {
    return new PartialAndTotalAdapter(function (str) { return option_1.traverse(parseCsv(str), function (x) { return adapter.applyTotal(x); }); }, // applyTotal
    function (// applyTotal
    arr) { return printCsv(arr.map(function (x) { return adapter.unapplyTotal(x); })); }, // unapplyTotal
    function (// unapplyTotal
    x) { return x.chain(function (str) { return option_1.traverse(parseCsv(str), function (x) { return adapter.applyTotal(x); }); }); }, // applyPartial
    function (// applyPartial
    arr) { return option_1.some(printCsv(arr.map(function (x) { return adapter.unapplyTotal(x); }))); });
}
exports.array = array;
function literals() {
    var literals = Array.isArray(arguments[0]) ? arguments[0] : arguments;
    var applyTotal = function (str) {
        for (var i = 0; i < literals.length; i++)
            if (literals[i] === str)
                return option_1.some(str);
        return option_1.none;
    };
    var unapplyTotal = function (x) { return x; };
    var applyPartial = function (maybeStr) { return maybeStr.chain(applyTotal); };
    var unapplyPartial = option_1.some;
    return new PartialAndTotalAdapter(applyTotal, unapplyTotal, applyPartial, unapplyPartial);
}
exports.literals = literals;
/** Create adapter that always succeeds with the given value */
function of(a) {
    var applyTotal = function () { return option_1.some(a); };
    var unapplyTotal = function () { return ''; };
    var applyPartial = applyTotal;
    var unapplyPartial = function () { return option_1.none; };
    return new PartialAndTotalAdapter(applyTotal, unapplyTotal, applyPartial, unapplyPartial);
}
exports.of = of;
...
function parseCsv(str) {
    ...
    for (var i = 0;; i++) {
        ...
        switch (str[i]) {
            case ',':
                escape ? (cur += ',', escape = false) : (output.push(cur), cur = '');
                break;
            case '\\':
                escape ? (cur += '\\', escape = false) : escape = true;
                break;
            default:
                cur += str[i];
                break;
...
var _moduleId = __webpack_require__(56);
...
var m=__webpack_require__(7),n=__webpack_require__(8),p=__webpack_require__(5),q="function"===typeof Symbol&&Symbol["for"],r=q?Symbol["for"]("react.element"):60103,t=q?Symbol["for"]("react.portal"):60106,u=q?Symbol["for"]("react.fragment"):60107,v=q?Symbol["for"]("react.strict_mode"):60108,w=q?Symbol["for"]("react.provider"):60109,x=q?Symbol["for"]("react.context"):60110,y=q?Symbol["for"]("react.async_mode"):60111,z=q?Symbol["for"]("react.forward_ref"):60112,A="function"===
...
*/
var ba=__webpack_require__(2),m=__webpack_require__(23),A=__webpack_require__(7),C=__webpack_require__(5),ea=__webpack_require__(24),fa=__webpack_require__(25),ha=__webpack_require__(26),ja=__webpack_require__(8);
...
exports.custom = parser_1.custom;
exports.extra = parser_1.extra;
exports.oneOf = parser_1.oneOf;
exports.params = parser_1.params;
exports.path = parser_1.path;
exports.segment = parser_1.segment;
exports.tag = parser_1.tag;
var parser = __webpack_require__(27);
exports.parser = parser;
var adapter_1 = __webpack_require__(28);
exports.AdapterBase = adapter_1.AdapterBase;
exports.NamedAdapter = adapter_1.NamedAdapter;
exports.PartialAdapter = adapter_1.PartialAdapter;
exports.PartialAndTotalAdapter = adapter_1.PartialAndTotalAdapter;
exports.TotalAdapter = adapter_1.TotalAdapter;
exports.array = adapter_1.array;
exports.boolean = adapter_1.boolean;
exports.date = adapter_1.date;
exports.int = adapter_1.int;
exports.literals = adapter_1.literals;
exports.nat = adapter_1.nat;
exports.nestring = adapter_1.nestring;
exports.of = adapter_1.of;
exports.partialAdapter = adapter_1.partialAdapter;
exports.partialAndTotalAdapter = adapter_1.partialAndTotalAdapter;
exports.string = adapter_1.string;
exports.totalAdapter = adapter_1.totalAdapter;
var adapter = __webpack_require__(28);
...
var jss_1 = __webpack_require__(3);
// component
var LifeTreeMenu = /** @class */ (function (_super) {
    __extends(LifeTreeMenu, _super);
    function LifeTreeMenu(props) {
        var _this = _super.call(this, props) || this;
        _this.toggleNodeVisibility = function (segments) { return function () {
            var visibility = _this.state.visibility;
            var root = visibility;
            var iter = !deepGet(root, segments);
            for (var i = segments.length - 1; i >= 0; i--) {
                var k = segments[i];
                var subSegments = segments.slice(0, i);
                var rest = deepGet(root, subSegments);
                iter = __assign({}, rest, (_a = {}, _a[k] = iter, _a));
                if (!iter[k])
                    delete iter[k];
            }
            _this.setState({ visibility: iter });
            function deepGet(o, path) {
                var iter = o;
                for (var _i = 0, path_1 = path; _i < path_1.length; _i++) {
                    var p = path_1[_i];
                    if (!iter.hasOwnProperty(p))
                        return undefined;
                    iter = iter[p];
                }
                return iter;
        ...
        _this.toggleExpandAll = function () {
            var visibility = _this.state.visibility;
            var visibilityNext = visibility === _this.expandAll ? _this.initialVisibility : _this.expandAll;
            _this.setState({ visibility: visibilityNext });
        };
        /** Expand the first level */
        _this.initialVisibility = function () {
            var output = {};
            var prefixTree = _1.parser.rules[0]['prefixTrie'];
            for (var k in prefixTree) {
                if (!prefixTree.hasOwnProperty(k) || k === '')
                    continue;
                output[k] = true;
            }
            return output;
        ...
        _this.expandAll = function () {
            var prefixTree = _1.parser.rules[0]['prefixTrie'];
            return go(prefixTree);
            function go(prefixTree) {
                var output = {};
                var isLeaf = true;
                for (var k in prefixTree) {
                    if (!prefixTree.hasOwnProperty(k) || k === '')
                        continue;
                    output[k] = go(prefixTree[k]);
                    isLeaf = false;
                }
                return isLeaf ? true : output;
            }
        }();
        _this.state = { visibility: _this.initialVisibility };
        return _this;
    }
    /** Using `PrefixTree`-structure to build hierarchical menu */
    LifeTreeMenu.prototype.renderTree = function () {
        var self = this;
        ...
        var yCoords = 0;
        var xCoords = 0;
        var unit = theme_1.default.spacing.unit;
        var itemHeight = unit * 4;
        var hStep = unit * 3;
        var vStep = unit * 3.5;
        traverseTreeRec(prefixTree, []);
        return React.createElement(React.Fragment, null,
            React.createElement("svg", { xmlns: "http://www.w3.org/2000/svg", width: xCoords * hStep + 240, height: yCoords * vStep + unit, shapeRendering: "crispEdges", className: classes.svg },
                React.createElement("defs", null,
                    React.createElement("g", { id: "minus", stroke: "rgba(0,0,0,0.25)", strokeWidth: "1" },
                        React.createElement("path", { d: "M0 0 h 8 v 8 h -8 z", fill: "#fff" }),
                        React.createElement("path", { d: "M2 4 h 5", fill: "none", strokeWidth: "1" })),
                    React.createElement("g", { id: "plus", stroke: "rgba(0,0,0,0.25)", fill: "none", strokeWidth: "1" },
                        React.createElement("use", { xlinkHref: "#minus" }),
                        React.createElement("path", { d: "M4 2 v 5", stroke: "rgba(0,0,0,0.25)", fill: "none", strokeWidth: "1" }))),
                React.createElement("g", { transform: "translate(0, " + unit + ")" },
                    svgObjects,
                    svgIcons)));
        function traverseTreeRec(prefixTree, stack, parentY) {
            if (parentY === void 0) { parentY = -1; }
            var route = _1.parser.parse(stack.join('/'));
            var childrenSegments = Object.keys(prefixTree).filter(function (x) { return x !== ''; });
            var hasChildren = !!childrenSegments.length;
            var visible = isVisible(state.visibility, stack);
            var key = yCoords;
            var currentParentY = yCoords;
            var textClass = [classes.text, route && self.props.route.tag === route.tag ? classes.active : undefined].filter(function (x) { return !!x; }).join(' ');
            route && (xCoords = Math.max(xCoords, stack.length - 1));
            // Horizontal dotted line
            route && stack.length > 1 && svgObjects.push(React.createElement("path", { key: "path-" + key, d: "M" + ((stack.length - 1) * hStep - (unit * 2)) + " " + yCoords * vStep + " h " + unit * 1.7, stroke: "rgba(0,0,0,0.25)", strokeDasharray: "1, 1", strokeWidth: "1" }));
            // Vertical dotted line
            route && stack.length > 1 && (parentY >= 0) && svgObjects.push(React.createElement("path", { key: "path-parent-" + key, d: "M" + ((stack.length - 1) * hStep - (unit * 2)) + " " + yCoords * vStep + " V " + (parentY * vStep + unit) + " ", stroke: "rgba(0,0,0,0.25)", strokeDasharray: "1, 1", strokeWidth: "1" }));
            // Fold/unfold icon
            route && stack.length > 1 && hasChildren && svgIcons.push(React.createElement("use", { key: "icon-" + key, xlinkHref: visible ? '#minus' : '#plus', x: (stack.length - 1) * hStep - 4 - (unit * 2), y: yCoords * vStep - 4, onClick: self.toggleNodeVisibility(stack) }));
            // @ts-ignore
            route && svgObjects.push(React.createElement("text", { className: textClass, x: (stack.length - 1) * hStep, y: yCoords * vStep + (unit * 0.8), key: "link-" + key },
                React.createElement("a", { xlinkHref: '#' + _1.parser.print(route) }, route.title)));
            route && yCoords++;
            var children = visible ? childrenSegments.map(function (key, idx) {
                return traverseTreeRec(prefixTree[key], stack.concat(key), idx === childrenSegments.length - 1 ? currentParentY : -1);
            }) : [];
            var itemClassName = [classes.item].filter(function (x) { return !!x; }).join(' ');
        }
        function isVisible(visibility, path) {
            var iter = visibility;
    ...
    /** Fold/unfold icon */
    LifeTreeMenu.prototype.renderPlusMinus = function (expanded, onClick) {
        return React.createElement("svg", { xmlns: "http://www.w3.org/2000/svg", width: "9", height: "9", shapeRendering: "crispEdges", className: classes.plusMinus, onClick: onClick },
            React.createElement("path", { d: "M0 0 h 8 v 8 h -8 z", stroke: "rgba(0,0,0,0.54)", fill: "none", strokeWidth: "1" }),
            React.createElement("path", { d: "M2 4 h 5", stroke: "rgba(0,0,0,0.54)", fill: "none", strokeWidth: "1" }),
            !expanded && React.createElement("path", { d: "M4 2 v 5", stroke: "rgba(0,0,0,0.54)", fill: "none", strokeWidth: "1" }));
    };
    LifeTreeMenu.prototype.render = function () {
        var _a = this.props, className = _a.className, route = _a.route, rest = __rest(_a, ["className", "route"]);
        var visibility = this.state.visibility;
        var rootClass = [this.props.className, classes.root].filter(function (x) { return !!x; }).join(' ');
        return React.createElement("div", __assign({}, rest, { className: rootClass }),
            React.createElement("a", { href: "javascript://void 0", onClick: this.toggleExpandAll, className: classes.expand }, visibility === this.expandAll ? 'Collapse all' : 'Expand all'),
            this.renderTree());
    };
    return LifeTreeMenu;
}(React.Component));
exports.default = LifeTreeMenu;
// styles
exports.styles = function () {
    var unit = theme_1.default.spacing.unit;
    return {
        ...
        expand: __assign({}, theme_1.linkStyle['& a'], { fontSize: 14, borderBottomStyle: 'dotted' }),
        item: {},
        hidden: {
            visibility: 'hidden',
        },
...
/* WEBPACK VAR INJECTION */}.call(__webpack_exports__, __webpack_require__(18), __webpack_require__(53)(module)))
...
var _PluginsRegistry = __webpack_require__(58);
...
var _functions = __webpack_require__(66);
...
var _KeyframesRule = __webpack_require__(61);
...
var _FontFaceRule = __webpack_require__(63);

var _FontFaceRule2 = _interopRequireDefault(_FontFaceRule);

var _ViewportRule = __webpack_require__(64);
...
var _jssTemplate = __webpack_require__(70);

var _jssTemplate2 = _interopRequireDefault(_jssTemplate);

var _jssGlobal = __webpack_require__(72);

var _jssGlobal2 = _interopRequireDefault(_jssGlobal);

var _jssExtend = __webpack_require__(73);

var _jssExtend2 = _interopRequireDefault(_jssExtend);

var _jssNested = __webpack_require__(74);

var _jssNested2 = _interopRequireDefault(_jssNested);

var _jssCompose = __webpack_require__(75);

var _jssCompose2 = _interopRequireDefault(_jssCompose);

var _jssCamelCase = __webpack_require__(76);

var _jssCamelCase2 = _interopRequireDefault(_jssCamelCase);

var _jssDefaultUnit = __webpack_require__(78);

var _jssDefaultUnit2 = _interopRequireDefault(_jssDefaultUnit);

var _jssExpand = __webpack_require__(80);

var _jssExpand2 = _interopRequireDefault(_jssExpand);

var _jssVendorPrefixer = __webpack_require__(82);

var _jssVendorPrefixer2 = _interopRequireDefault(_jssVendorPrefixer);

var _jssPropsSort = __webpack_require__(87);
...
var _hyphenateStyleName = __webpack_require__(77);
...
var _defaultUnits = __webpack_require__(79);
...
var _props = __webpack_require__(81);
...
function Spinner(props) {
    var className = props.className, overlay = props.overlay, overlayClassProp = props.overlayClass, rest = __rest(props, ["className", "overlay", "overlayClass"]);
    var spinnerClass = [className, classes.spinner].filter(function (x) { return !!x; }).join(' ');
    var overlayClass = [overlayClassProp, classes.overlay].filter(function (x) { return !!x; }).join(' ');
    var spinner = React.createElement("div", __assign({}, rest, { className: spinnerClass }));
    return overlay ? React.createElement("div", { className: overlayClass }, spinner) : spinner;
}
...
var Breadcrumbs = /** @class */ (function (_super) {
    ...
    function Breadcrumbs() {
        var _this = _super !== null && _super.apply(this, arguments) || this;
        _this.toolbarAnchor = null;
        return _this;
    }
    Breadcrumbs.prototype.componentWillMount = function () {
        this.toolbarAnchor = document.getElementById('toolbar_anchor');
    };
    Breadcrumbs.prototype.render = function () {
        var _a = this.props, breadcrumbs = _a.breadcrumbs, className = _a.className, toolbarClassName = _a.toolbarClassName, embeddedClassName = _a.embeddedClassName;
        var rootClass = [className, classes.root].concat((this.toolbarAnchor ? [classes.toolbar, toolbarClassName] : [embeddedClassName])).filter(function (x) { return !!x; }).join(' ');
        var contents = React.createElement("div", { className: rootClass }, intersperse(breadcrumbs.map(function (r, idx) { return React.createElement("a", { key: idx, href: '#' + _1.parser.print(r) }, r.title); }).reverse(), React.createElement("span", null, " \u2192 ")));
...
var unit = theme_1.default.spacing.unit;
exports.styles = {
    root: __assign({ fontSize: 14 }, theme_1.linkStyle),
};
var classes = jss_1.default.createStyleSheet(exports.styles).attach().classes;
...
module.exports = "<p>This demo shows how <code>Parser.prototype.parseAll</code> can be used to\nimplement simple «breadcrumbs» navigation strategy. <code>parseAll</code> differs\nfrom <code>parse</code> by the fact that its result contains all routes that were\nsuccessfully matched against the input, including those that left some\nsegments unconsumed.</p>\n<pre><code class=\"lang-ts\">const parser = r.oneOf(\n  r.tag(&#39;Home&#39;).path(&#39;/&#39;),\n  r.tag(&#39;Shop&#39;).path(&#39;/shop&#39;),\n  r.tag(&#39;Item&#39;).path(&#39;/shop/item&#39;).segment(&#39;id&#39;, r.nestring),\n);\n\nconsole.log(parser.parse(&#39;/shop/item/42&#39;));\n// =&gt; { tag: &#39;Item&#39;, id: &#39;42&#39; }\nconsole.log(parser.parseAll(&#39;/shop/item/42&#39;));\n// =&gt; [{ tag: &#39;Item&#39;, id: &#39;42&#39; }, { tag: &#39;Shop&#39; }, { tag: &#39;Home&#39; }]\nconsole.log(parser.parseAll(&#39;/unknown-url&#39;));\n// =&gt; []\n</code></pre>\n<p>If the input didn&#39;t match any of the routes, method <code>parseAll</code> returns\nempty array, otherwise the first element would be the best match (the\none that has less unconsumed segments), then the second best and so\non. In other words, the output has the reverse order compared to the\norder breadcrumbs are usually displayed.</p>\n<p>Since results of <code>parseAll</code> contain only successfully matched routes,\neach descendant route has to have a common path prefix with its\nparent. This condition restricts the use of <code>parseAll</code> as a\nbreadcrumbs implementation only to these simple cases.</p>\n";
